...
type JabatanKoperasi struct {
	...
	Tingkat   string         `gorm:"type:enum('pengurus','pengawas','anggota');not null" json:"tingkat"`
...
type AnggotaKoperasi struct {
	...
	Posisi         string     `gorm:"type:enum('pengurus','pengawas','anggota');default:'anggota'" json:"posisi"`
	JabatanID      uint64     `json:"jabatan_id"`
	TanggalMasuk   *time.Time `json:"tanggal_masuk"`
	TanggalKeluar  *time.Time `json:"tanggal_keluar"`
	StatusAnggota  string     `gorm:"type:enum('aktif','non_aktif','keluar');default:'aktif';index" json:"status_anggota"`
	...
	KlinikPasien           KlinikPasien              `gorm:"foreignKey:AnggotaID" json:"klinik_pasien,omitempty"`
